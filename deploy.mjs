...
const runtimeDir = path.join('/tmp', 'waqf-alathar-v5-runtime');
  ...
  try {
    ...
    if (process.env.ADMIN_USERNAME && process.env.ADMIN_PASSWORD) {
      ...
      if (cookie) {
        ...
        const tokenMatch = adminHtml.match(/window\.WAQF_CMS_TOKEN=("(?:\\.|[^"\\])*")/);
        if (tokenMatch) {
          const cmsToken = JSON.parse(tokenMatch[1]);
          statuses.cmsBootstrap = (await fetch(`${base}/api/cms/bootstrap`, {
            redirect: 'manual', headers: { Cookie: cookie, 'x-waqf-cms-token': cmsToken },
          })).status;
        }
      }
    }
